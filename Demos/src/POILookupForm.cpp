/*
  POILookupForm - a demo program for libosmscout
...
int main(int argc, char* argv[])
{
  ...
  osmscout::POIFormSearchParameter searchParameter;

  searchParameter.SetAdminRegionSearchString(args.adminRegion);
  searchParameter.SetPOISearchString(args.poi);
  ...
  std::cout << "POI:                     " << searchParameter.GetPOISearchString() << std::endl;
  ...
  if (!locationService->SearchForPOIByForm(searchParameter,
                                           searchResult)) {
    std::cerr << "Error while searching for POI" << std::endl;
    return 1;
  }

  locationSearchTime.Stop();

  std::cout << "POI search time: " << locationSearchTime.ResultString() << std::endl;
  std::cout << std::endl;
