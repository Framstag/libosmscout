...
namespace osmscout {
  AreaInAreaIndex::AreaInAreaIndex(const std::string& filename)
  : filepart(filename)
  {
    // no code
  }

  bool AreaInAreaIndex::Load(const std::string& path)
  {
    FileScanner scanner;
    ...
    while (l>=0) {
      ...
      for (size_t i=0; i<entries; i++) {
        ...
        if (!scanner.GetPos(offset)) {
          std::cerr << "Cannot read data, level " << l << " entry " << i << std::endl;
          return false;
        }
  ...
  void AreaInAreaIndex::GetOffsets(const StyleConfig& styleConfig,
    ...
    for (size_t level=0; level<=maxLevel && level<=this->maxLevel && co.size()>0; level++) {
      ...
      for (size_t i=0; i<co.size(); i++) {
        ...
        if (offsets.size()+entry->second.dataOffsets.size()>maxCount) {
          std::cout << "Found " << offsets.size() << " offsets in '" << filepart << "' with maximum level " << maxLevel << std::endl;
          return;
        }
  ...
  void AreaInAreaIndex::DumpStatistics()
  {
    // TODO
  }

  WayInAreaIndex::WayInAreaIndex(const std::string& filename)
  : filepart(filename)
  {
    // no code
  }

  bool WayInAreaIndex::Load(const std::string& path)
  {
    FileScanner scanner;
    std::string filename=path+"/"+filepart;
  ...
  void WayInAreaIndex::GetOffsets(const StyleConfig& styleConfig,
                             double minlon, double minlat,
                             double maxlon, double maxlat,
  ...
  void WayInAreaIndex::DumpStatistics()
  {
    // TODO
  }
}
