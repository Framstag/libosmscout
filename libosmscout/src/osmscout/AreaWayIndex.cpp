...
namespace osmscout {
    ...
    try {
      ...
      for (size_t i=0; i<indexEntries; i++) {
        ...
        if (data.bitmapOffset>0) {
          ...
          data.cellDimension=cellDimension[data.indexLevel];

          data.boundingBox=GeoBox(GeoCoord(data.cellYStart*data.cellDimension.height-90.0,
                                           data.cellXStart*data.cellDimension.width-180.0),
                                  GeoCoord((data.cellYEnd+1)*data.cellDimension.height-90.0,
                                           (data.cellXEnd+1)*data.cellDimension.width-180.0));
        }
                                ...
                                std::unordered_set<FileOffset>& offsets) const
  {
    ...
    uint32_t minxc=(uint32_t)floor((boundingBox.GetMinLon()+180.0)/typeData.cellDimension.width);
    uint32_t maxxc=(uint32_t)floor((boundingBox.GetMaxLon()+180.0)/typeData.cellDimension.width);

    uint32_t minyc=(uint32_t)floor((boundingBox.GetMinLat()+90.0)/typeData.cellDimension.height);
    uint32_t maxyc=(uint32_t)floor((boundingBox.GetMaxLat()+90.0)/typeData.cellDimension.height);
