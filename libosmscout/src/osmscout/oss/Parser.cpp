...
void Parser::WAYSTYLE() {
		...
		while (la->kind == 12 || la->kind == 19 || la->kind == 23) {
			if (la->kind == 12) {
				LineStyle lineStyle; 
				LINEDEF(lineStyle);
...
void Parser::AREASTYLE() {
		...
		while (!(la->kind == 0 || la->kind == 11)) {SynErr(65); Get();}
		Expect(11);
		Expect(5);
		name=Destring(t->val); 
...
void Parser::REFDEF(LabelStyle& style) {
		while (!(la->kind == 0 || la->kind == 23)) {SynErr(68); Get();}
		Expect(23);
		if (la->kind == 13) {
			while (!(la->kind == 0 || la->kind == 13)) {SynErr(69); Get();}
			Get();
			if (la->kind == 26) {
				LabelStyle::Style s; 
				LABELSTYLE(s);
...
void Parser::SYMBOLDEF(SymbolStyle& style) {
		...
		while (!(la->kind == 0 || la->kind == 24)) {SynErr(70); Get();}
		Expect(24);
		SYMBOLSTYLE(s);
		style.SetStyle(s); 
		COLOR(r,g,b,a);
		...
		style.SetSize(size); 
		if (la->kind == 13) {
			while (!(la->kind == 0 || la->kind == 13)) {SynErr(71); Get();}
			Get();
			if (la->kind == 40) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
...
void Parser::ICONDEF(IconStyle& style) {
		while (!(la->kind == 0 || la->kind == 25)) {SynErr(72); Get();}
		Expect(25);
		Expect(1);
		style.SetIconName(t->val); 
		if (la->kind == 13) {
			while (!(la->kind == 0 || la->kind == 13)) {SynErr(73); Get();}
			Get();
			if (la->kind == 40) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
...
void Parser::FILLDEF(FillStyle& style, std::string& filter) {
		...
		while (!(la->kind == 0 || la->kind == 16)) {SynErr(76); Get();}
		Expect(16);
		COLOR(r,g,b,a);
...
void Parser::PATTERNDEF(PatternStyle& style) {
		...
		if (la->kind == 13) {
			while (!(la->kind == 0 || la->kind == 13)) {SynErr(79); Get();}
			Get();
			if (la->kind == 40) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
...
void Parser::BORDERDEF(LineStyle& style) {
		...
		while (!(la->kind == 0 || la->kind == 18)) {SynErr(80); Get();}
		Expect(18);
		COLOR(r,g,b,a);
		style.SetLineColor(r,g,b,a); 
		if (la->kind == 13) {
			while (!(la->kind == 0 || la->kind == 13)) {SynErr(81); Get();}
			Get();
			if (la->kind == 26) {
				LineStyle::Style s=style.GetStyle(); 
				LINESTYLE(s);
...
bool Parser::StartOf(int s)
{
  ...
  const bool x = false;
	...
	static bool set[3][62] = {
		{T,x,x,x, x,x,T,x, T,T,x,T, T,T,x,x, T,T,T,T, x,x,x,T, T,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,T, T,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,T,T,T, x,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x}
	};
...
void Errors::SynErr(int line, int col, int n)
{
  ...
  switch (n) {
			...
			case 16: s = coco_string_create("\"FILL\" expected"); break;
			case 17: s = coco_string_create("\"PATTERN\" expected"); break;
			case 18: s = coco_string_create("\"BORDER\" expected"); break;
			case 19: s = coco_string_create("\"LABEL\" expected"); break;
			case 20: s = coco_string_create("\"COLOR\" expected"); break;
			case 21: s = coco_string_create("\"BGCOLOR\" expected"); break;
			case 22: s = coco_string_create("\"BORDERCOLOR\" expected"); break;
			case 23: s = coco_string_create("\"REF\" expected"); break;
			case 24: s = coco_string_create("\"SYMBOL\" expected"); break;
			case 25: s = coco_string_create("\"ICON\" expected"); break;
			case 26: s = coco_string_create("\"STYLE\" expected"); break;
			case 27: s = coco_string_create("\"none\" expected"); break;
			case 28: s = coco_string_create("\"normal\" expected"); break;
			case 29: s = coco_string_create("\"longDash\" expected"); break;
			case 30: s = coco_string_create("\"dotted\" expected"); break;
			case 31: s = coco_string_create("\"lineDot\" expected"); break;
			case 32: s = coco_string_create("\"contour\" expected"); break;
			case 33: s = coco_string_create("\"plate\" expected"); break;
			case 34: s = coco_string_create("\"emphasize\" expected"); break;
			case 35: s = coco_string_create("\"box\" expected"); break;
			case 36: s = coco_string_create("\"triangle\" expected"); break;
			case 37: s = coco_string_create("\"circle\" expected"); break;
			case 38: s = coco_string_create("\"LAYER\" expected"); break;
			case 39: s = coco_string_create("\"FILTER\" expected"); break;
			case 40: s = coco_string_create("\"MINMAG\" expected"); break;
