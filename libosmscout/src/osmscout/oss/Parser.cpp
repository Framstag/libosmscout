...
void Parser::OSS() {
		...
		Expect(6);
		while (la->kind == 8 || la->kind == 11 || la->kind == 13) {
			STYLE();
		}
		Expect(7);
}
...
void Parser::NODESTYLE() {
		...
		type=config.GetTypeConfig()->GetNodeTypeId(config.GetTypeConfig()->GetTagId(key.c_str()),
		                                         value.c_str());
		
		                  if (type==typeIgnore) {
		                    std::string e="Unknown type '"+key+"/"+value+"'";
...
void Parser::WAYSTYLE() {
		...
		Expect(12);
		Expect(2);
		type=config.GetTypeConfig()->GetWayTypeId(config.GetTypeConfig()->GetTagId(key.c_str()),
		                                         value.c_str());
		...
		while (la->kind == 14 || la->kind == 21 || la->kind == 25) {
			if (la->kind == 14) {
				LineStyle lineStyle; 
				LINEDEF(lineStyle);
...
void Parser::AREASTYLE() {
		...
		while (!(la->kind == 0 || la->kind == 13)) {SynErr(67); Get();}
		Expect(13);
		Expect(9);
		Expect(5);
		key=Destring(t->val); 
		...
		Expect(5);
		value=Destring(t->val); 
		type=config.GetTypeConfig()->GetAreaTypeId(config.GetTypeConfig()->GetTagId(key.c_str()),
		                                          value.c_str());
...
void Parser::LABELDEF(LabelStyle& style) {
		while (!(la->kind == 0 || la->kind == 21)) {SynErr(68); Get();}
		Expect(21);
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(69); Get();}
			Get();
			if (la->kind == 28) {
				LabelStyle::Style s; 
				LABELSTYLE(s);
...
void Parser::REFDEF(LabelStyle& style) {
		while (!(la->kind == 0 || la->kind == 25)) {SynErr(70); Get();}
		Expect(25);
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(71); Get();}
			Get();
			if (la->kind == 28) {
				LabelStyle::Style s; 
				LABELSTYLE(s);
...
void Parser::SYMBOLDEF(SymbolStyle& style) {
		...
		style.SetSize(size); 
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(73); Get();}
			Get();
			if (la->kind == 42) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
...
void Parser::ICONDEF(IconStyle& style) {
		...
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(75); Get();}
			Get();
			if (la->kind == 42) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
...
void Parser::LINEDEF(LineStyle& style) {
		...
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(77); Get();}
			Get();
			if (la->kind == 28) {
...
void Parser::FILLDEF(FillStyle& style, std::string& filter) {
		...
		while (!(la->kind == 0 || la->kind == 18)) {SynErr(78); Get();}
		Expect(18);
		COLOR(r,g,b,a);
		style.SetColor(r,g,b,a); 
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(79); Get();}
			Get();
			if (la->kind == 40) {
...
void Parser::PATTERNDEF(PatternStyle& style) {
		while (!(la->kind == 0 || la->kind == 19)) {SynErr(80); Get();}
		Expect(19);
		Expect(5);
		style.SetPattern(Destring(t->val)); 
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(81); Get();}
			Get();
			if (la->kind == 42) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
...
void Parser::BORDERDEF(LineStyle& style) {
		...
		while (!(la->kind == 0 || la->kind == 20)) {SynErr(82); Get();}
		Expect(20);
		COLOR(r,g,b,a);
		style.SetLineColor(r,g,b,a); 
		if (la->kind == 15) {
			while (!(la->kind == 0 || la->kind == 15)) {SynErr(83); Get();}
			Get();
			if (la->kind == 28) {
				LineStyle::Style s=style.GetStyle(); 
				LINESTYLE(s);
...
bool Parser::StartOf(int s)
{
  ...
  const bool x = false;
	...
	static bool set[3][64] = {
		{T,x,x,x, x,x,T,x, T,x,x,T, x,T,T,T, x,x,T,T, T,T,x,x, x,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,x,x, x,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,x,x, x,x,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x}
	};
...
void Errors::SynErr(int line, int col, int n)
{
  ...
  switch (n) {
			...
			case 9: s = coco_string_create("\"KEY\" expected"); break;
			case 10: s = coco_string_create("\"VALUE\" expected"); break;
			case 11: s = coco_string_create("\"WAY\" expected"); break;
			case 12: s = coco_string_create("\"PRIO\" expected"); break;
			case 13: s = coco_string_create("\"AREA\" expected"); break;
			case 14: s = coco_string_create("\"LINE\" expected"); break;
			case 15: s = coco_string_create("\"WITH\" expected"); break;
			case 16: s = coco_string_create("\"ALTCOLOR\" expected"); break;
			case 17: s = coco_string_create("\"OUTLINECOLOR\" expected"); break;
			case 18: s = coco_string_create("\"FILL\" expected"); break;
			case 19: s = coco_string_create("\"PATTERN\" expected"); break;
			case 20: s = coco_string_create("\"BORDER\" expected"); break;
			case 21: s = coco_string_create("\"LABEL\" expected"); break;
			case 22: s = coco_string_create("\"COLOR\" expected"); break;
			case 23: s = coco_string_create("\"BGCOLOR\" expected"); break;
			case 24: s = coco_string_create("\"BORDERCOLOR\" expected"); break;
			case 25: s = coco_string_create("\"REF\" expected"); break;
			case 26: s = coco_string_create("\"SYMBOL\" expected"); break;
			case 27: s = coco_string_create("\"ICON\" expected"); break;
			case 28: s = coco_string_create("\"STYLE\" expected"); break;
			case 29: s = coco_string_create("\"none\" expected"); break;
			case 30: s = coco_string_create("\"normal\" expected"); break;
			case 31: s = coco_string_create("\"longDash\" expected"); break;
			case 32: s = coco_string_create("\"dotted\" expected"); break;
			case 33: s = coco_string_create("\"lineDot\" expected"); break;
			case 34: s = coco_string_create("\"contour\" expected"); break;
			case 35: s = coco_string_create("\"plate\" expected"); break;
			case 36: s = coco_string_create("\"emphasize\" expected"); break;
			...
			case 41: s = coco_string_create("\"FILTER\" expected"); break;
			case 42: s = coco_string_create("\"MINMAG\" expected"); break;
			case 43: s = coco_string_create("\"MAXMAG\" expected"); break;
			case 44: s = coco_string_create("\"FADE\" expected"); break;
			case 45: s = coco_string_create("\"AT\" expected"); break;
			case 46: s = coco_string_create("\"world\" expected"); break;
			...
			case 59: s = coco_string_create("\"MINPIXEL\" expected"); break;
			case 60: s = coco_string_create("\"WIDTH\" expected"); break;
			case 61: s = coco_string_create("\"OUTLINE\" expected"); break;
			...
			case 84: s = coco_string_create("invalid LINESTYLE"); break;
