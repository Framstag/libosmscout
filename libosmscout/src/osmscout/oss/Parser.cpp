...
void Parser::LABELDEF(LabelStyle& style) {
		while (!(la->kind == 0 || la->kind == 23)) {SynErr(66); Get();}
		Expect(23);
...
void Parser::REFDEF(LabelStyle& style) {
		while (!(la->kind == 0 || la->kind == 27)) {SynErr(68); Get();}
		Expect(27);
		if (la->kind == 14) {
...
void Parser::SYMBOLDEF(SymbolStyle& style) {
		...
		while (!(la->kind == 0 || la->kind == 28)) {SynErr(70); Get();}
		Expect(28);
		SYMBOLSTYLE(s);
		style.SetStyle(s); 
		COLOR(r,g,b,a);
...
void Parser::ICONDEF(IconStyle& style) {
		while (!(la->kind == 0 || la->kind == 29)) {SynErr(72); Get();}
		Expect(29);
		Expect(1);
...
void Parser::FILLDEF(FillStyle& style) {
		...
		while (!(la->kind == 0 || la->kind == 20)) {SynErr(77); Get();}
		Expect(20);
		COLOR(r,g,b,a);
		style.SetFillColor(r,g,b,a); 
		if (la->kind == 14) {
		...
		if (la->kind == 21) {
			...
			if (la->kind == 14) {
				while (!(la->kind == 0 || la->kind == 14)) {SynErr(80); Get();}
				Get();
				if (la->kind == 17) {
					double dash; 
					Get();
...
void Parser::PATTERNDEF(PatternStyle& style) {
		...
		while (!(la->kind == 0 || la->kind == 22)) {SynErr(81); Get();}
		Expect(22);
		Expect(5);
		style.SetPattern(Destring(t->val)); 
		if (la->kind == 14) {
			while (!(la->kind == 0 || la->kind == 14)) {SynErr(82); Get();}
			Get();
			if (la->kind == 11) {
				Mag minMag=style.GetMinMag(); 
				MINMAG(minMag);
		...
		if (la->kind == 21) {
			...
			style.SetBorderColor(r,g,b,a); 
			if (la->kind == 14) {
				while (!(la->kind == 0 || la->kind == 14)) {SynErr(84); Get();}
				Get();
				if (la->kind == 17) {
					double dash; 
					Get();
...
bool Parser::StartOf(int s)
{
  ...
  const bool x = false;
	...
	static bool set[3][62] = {
		{T,x,x,x, x,x,T,x, T,T,x,x, T,T,T,x, x,x,x,x, T,T,T,T, x,x,x,T, T,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,T, T,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,T,T, x,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x}
	};
...
void Errors::SynErr(int line, int col, int n)
{
  ...
  switch (n) {
			...
			case 26: s = coco_string_create("\"BORDERCOLOR\" expected"); break;
			case 27: s = coco_string_create("\"REF\" expected"); break;
			case 28: s = coco_string_create("\"SYMBOL\" expected"); break;
			case 29: s = coco_string_create("\"ICON\" expected"); break;
			case 30: s = coco_string_create("\"STYLE\" expected"); break;
			case 31: s = coco_string_create("\"normal\" expected"); break;
			...
			case 35: s = coco_string_create("\"none\" expected"); break;
			...
			case 86: s = coco_string_create("invalid LABELSTYLE"); break;
			case 87: s = coco_string_create("invalid SYMBOLSTYLE"); break;
