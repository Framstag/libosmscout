...
namespace osmscout {
  ...
  void FileWriter::Write(const std::vector<Point>& nodes, bool writeIds)
  {
    ...
    if (writeIds) {
      ...
      if (nodesSize<16) { // 2^4 (8 -3 flags -1 continuation bit)
        uint8_t nodeSize1=(nodesSize & 0x0f) << 3;
        uint8_t size=coordSizeFlags | hasNodesFlags | nodeSize1;

        Write(size);
      }
      else if (nodesSize<2048) { // 2^(4+7)=2^11 (16-3-1-1)
        uint8_t size[2];
      ...
      else if (nodesSize<262144) { // 2^(11+7)=2^18 (24-3-1-1-1)
        uint8_t size[3];
        uint8_t nodeSize1=((nodesSize & 0x0f) << 3) | 0x80; // The initial 4 bits + continuation bit
      ...
      if (nodesSize<32) {    // 2^5 (8 bits - 2 flag bit -1 continuation bit)
        uint8_t nodeSize1=(nodesSize & 0x1f) << 2;
        uint8_t size=coordSizeFlags | nodeSize1;

        Write(size);
      }
      else if (nodesSize<4096) { // 2^(5+7)=2^12 (16-2-1-1)
        uint8_t size[2];
      ...
      else if (nodesSize<524288) { // 2^(12+7)=2^19 (24-2-1-1-1)
        uint8_t size[3];
        uint8_t nodeSize1=((nodesSize & 0x1f) << 2) | 0x80; // The initial 5 bits + continuation bit
