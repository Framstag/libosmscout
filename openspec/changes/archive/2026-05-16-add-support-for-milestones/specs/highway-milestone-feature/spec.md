...
The system SHALL store the OSM `distance` tag value as a `uint32_t` representing meters (decimal part truncated). Only simple decimal numbers with "." as separator and implicit meter unit are accepted. Other formats (unit suffixes, km+meters format, comma separator) SHALL log a warning and skip.

#### Scenario: Milestone with simple numeric distance in meters
- **WHEN** a node has `highway=milestone`, `distance=35`, and `ref=A2`
- **THEN** the distance field SHALL be `35` (meters)

#### Scenario: Milestone with decimal distance in meters
- **WHEN** a node has `highway=milestone`, `distance=35.5`, and `ref=A2`
- **THEN** the distance field SHALL be `35` (meters, decimal truncated)
...
The `GetLabel()` method on the value SHALL return the distance value formatted using the locale with a "m" unit suffix.

#### Scenario: GetLabel returns distance with meter unit
- **WHEN** `GetLabel(locale, 0)` is called on a HighwayMilestoneFeatureValue with distance=35
- **THEN** it SHALL return `"35 m"` (formatted per locale)
...
The system SHALL include a `HighwayMilestoneDescriptionProcessor` registered in `DescriptionService` that adds description entries for distance, ref, carriageway_ref, and marker values when a `HighwayMilestoneFeatureValue` is present.

#### Scenario: Processor renders distance and ref
- **WHEN** a buffer contains HighwayMilestoneFeatureValue with distance=35 and ref="A2"
- **THEN** the description SHALL contain a "HighwayMilestone" section with entries for distance ("35") and ref ("A2")

#### Scenario: Processor skips empty carriageway_ref and marker
- **WHEN** a buffer contains HighwayMilestoneFeatureValue with distance=35 and ref="A2" but empty carriageway_ref and marker
...
- **WHEN** a buffer contains HighwayMilestoneFeatureValue with distance=35, ref="A2", carriageway_ref="W", and marker="stone"
