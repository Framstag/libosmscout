...
- **THEN** the `onCurrentRoadInfo` callback is invoked with the road's name, ref, and type

#### Scenario: Road has no name
- **GIVEN** an active navigation session
- **WHEN** the vehicle is on a road without a name tag
- **THEN** the `onCurrentRoadInfo` callback is invoked with an empty name
- **AND** the ref and type are still provided if available

#### Scenario: Vehicle off-route
- **GIVEN** an active navigation session
- **WHEN** the vehicle is off the planned route
- **THEN** the `onCurrentRoadInfo` callback is invoked with null or an empty info object
- **AND** the road info overlay is hidden
...
- **AND** the overlay is visually distinct from the next-turn instructions (smaller font, muted color)
...
- **AND** parses the returned "General" section entries for "Ref", "Type", and "Name" labels
