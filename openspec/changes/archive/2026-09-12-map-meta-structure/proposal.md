...
- Add a second container image (nginx-based) that serves exactly the public part of the repository (`public/`) over plain HTTP without authentication, and a docker-compose file that orchestrates it with the regeneration container on one shared repository volume.
- Add a client integration guide (`Documentation/MapClientGuide.md`) describing discovery, the own-version update check, and download verification.
...
- `web-server`: Contract of the read-only web server image (served surface exactly `public/`, anonymous access, no write path, JSON content types) and the compose orchestration of regeneration container (writer) and web server (reader) on one repository volume.
...
- `libosmscout/include/osmscout/io/Crc32.h`, `libosmscout/src/osmscout/io/Crc32.cpp` — new public CRC-32 API in the core library (`osmscout::Crc32`, `osmscout::ComputeFileCrc32`), used by the metadata emitter.
- New: regeneration script (bash), container image definition, server directory layout definition, client update-check documentation — all living in this change's deliverables unless a home is chosen in design. Additionally a nginx-based serve image, a docker-compose orchestration file, and a client integration guide (`Documentation/MapClientGuide.md`).
