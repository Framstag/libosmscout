...
**Reason**: The regeneration pipeline produces a single basemap, so there is no variant to choose
between. Variant selection would offer the user a choice that cannot exist.
**Migration**: The Map Download dialog offers the single available basemap directly; the control
that presented variants is removed. Should a second basemap variant be produced later, it needs a
new requirement describing how it is distinguished, not a reinstatement of this one.
