...
## 15. Rebase onto `master` (`8a497449e`)

The branch was behind `master` by 172 commits and could not be merged, so it was rebased onto
`8a497449e` (the route presentation colours merge).

Conflicts and their resolution:

- `TODO.md`, `Tests/CMakeLists.txt` and `Tests/meson.build`: both sides added a test registration
  and a TODO section, so both sides are kept - the extended uncrustify-drift entry of this
  branch, the `map-painter-visible-object-cull` section of `master`, and the registrations of
  `MapPainterLabelCullingTest` (master) and `MapPainterLabelReuseTest` (this branch).
- `libosmscout-map/src/osmscoutmap/MapPainter.cpp`: `master` added `maxWidthFeatureWidth` next to
  the `GetGridPoints` declaration this branch rewrites; the constant is kept and the declaration
  of this branch (reused buffer instead of a `std::set` return) is taken.
- `libosmscout-map/include/osmscoutmap/LabelLayouterHelper.h`: the helpers of `master`
  (`CountLabelWords`, `GetMaxLabelPaddingPixel`, `GetLabelLayoutMarginPixel`,
  `GetLabelExtentBound`) and `BuildMeasurementEnvironment` of this branch; both are kept.
- `libosmscout-map/include/osmscoutmap/LabelLayouter.h`: `Reset()` clears the layout viewport
  mark of `master` and the measurement slot of this branch; both are kept.

Two integration defects of the rebase surfaced in the suite:

- `Tests/src/MapPainterLabelCullingTest.cpp` returned a layouter by value (`MakeLayouter`). The
  layouter reuses its frame state (`LayoutJob` with its overlap canvases and the masks that point
  into them), so it is neither copyable nor movable now. The helper is replaced by
  `ConfigureLayouter(TestLayouter&)` and the seven test cases of the file construct their
  layouter in place.
- The same test expected one measurement per registered label
  (`REQUIRE(textLayouter.LayoutCalls()==reachTouches)`), which the measurement cache of this
  branch removes: the thirteen registrations of the same label are one measurement. The culling
  property the case guards is kept (a label whose reach cannot touch the layout viewport is
  neither stored nor measured, checked per registration now) and the reuse is asserted explicitly
  (`REQUIRE(textLayouter.LayoutCalls()==1)`). The file is uncrustify clean (drift 0).

Verification after the rebase: `cmake --build build` of all configured targets without a failure,
`ctest -j 4` with `QT_QPA_PLATFORM=offscreen` 132/132 passed, `meson compile -C build-meson` and
`meson test --timeout-multiplier 2 -C build-meson` 132 ok / 0 failed.
