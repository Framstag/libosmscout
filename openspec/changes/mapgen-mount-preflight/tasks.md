...
- [ ] 1.2 Extend the smoke checks with the unseeded case: a work area owned by root, started in the scheduled mode, has to stop with the actionable message (spec `mapgen-container`: a work area owned by another identity). Verify: on the runner the check passes, and it fails if the message loses the area name, the identity or the `chown` command
- [ ] 1.3 Keep the existing smoke checks green: single pass, scheduled mode, unusable schedule and the argument runs (spec `mapgen-container`: writable areas start normally). Verify: on the runner those checks pass in the same run as the new one
...
- [ ] 3.2 Verify the whole path on a runner: the unseeded work area stops the container with the actionable message, the writable cases still start and pass their checks, and the argument runs stay unaffected (spec `mapgen-container`: all four scenarios). Verify: a pull request run whose steps all conclude `success`
