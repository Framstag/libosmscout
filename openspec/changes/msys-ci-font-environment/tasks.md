...
- [x] 2.1 Add a font-provisioning step to both jobs in `.github/workflows/build_and test_on_msys.yml`, after the MSYS2 setup and before the build: copy `libosmscout-map-opengl/data/fonts/LiberationSans-Regular.ttf` into the MSYS2 prefix font directory and refresh the font cache for that directory only. Verify: the step appears in both jobs in the workflow diff and the workflow parses as YAML. (Spec: "MSYS jobs provide the font the font-dependent tests measure against", "MSYS font provisioning uses only repository content and the existing package setup".)
...
- [ ] 2.3 Read the verification output of the first CI run on the branch and decide the mechanism: if the family resolves to the bundled file, keep the prefix-copy provisioning; if it does not, replace the mechanism with the generated font configuration from `design.md` - D2 (bundled directory plus the Windows font directory) and re-verify. Verify: the decision and the observed resolved font file are recorded in `design.md` - Open Questions, and the verification step passes in both jobs. (Spec: "Named family resolves to the repository font".)
