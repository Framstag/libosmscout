...
- [ ] 6.2 Verify the non-release path end to end (spec `container-image-publishing`: a pull request publishes nothing, a failing verification stops publication). Verify: a pull-request run performs the build, all smoke checks and the compose smoke job and publishes nothing, and a `workflow_dispatch` run with publishing disabled starts on demand and also publishes nothing (the tag set it would publish is verified by extracting the tag step, since a run with publishing disabled skips the publish job)
...
## 7. Release publication through the release workflow

The release workflows create the release with the default `GITHUB_TOKEN`, and GitHub starts no workflow
run for events caused by that token, so the `release: published` trigger never reached the image workflow
(design decision 9). The release workflow therefore starts the publication itself.

- [x] 7.1 Give the image workflow a `release` input that publishes the release tag set (the given release version, the library version and `latest`), while a plain manual run keeps publishing a single tag and never `latest`, and reject a release publish whose tag equals the library version (spec `container-image-publishing`: tag set published for a release; spec `release-library-version`: the library version is not the release version). Verify: the tag step extracted from the workflow and run for the release-event path with and without a leading `v`, the plain manual tag, `release=true` with and without a leading `v`, and the rejections (no tag, `latest` as tag, invalid tag, tag equal to the library version)
- [x] 7.2 Dispatch the image workflow from `.github/workflows/release.yml` after the release has been created, on the released tag, with `actions: write` and a lookup that fails loudly when the tag cannot be determined (spec `container-image-publishing`: a release publishes all three tags). Verify: `bash -n` on the extracted step, the release lookup returning `v2024.06.02.1` for the version `2024.06.02.1` against the real repository, and a `workflow_dispatch` run of the image workflow on master starting on demand
- [x] 7.3 Document both publication paths and why the release workflow dispatches (spec `container-image-publishing`: the tag semantics are documented). Verify: section 5 of `Documentation/MapRepository.md` names the dispatch, the event trigger and the ref the dispatch uses
- [ ] 7.4 Verify the dispatch end to end on a real release: the release run dispatches the image workflow, which publishes the release tag, the library version tag and `latest` (spec `container-image-publishing`: tag set published for a release; 6.3 and 6.4 depend on this). Verify: the release run's summary names the release tag, the dispatched image run succeeds, and the three tags exist for both images
...
Still outstanding on a runner: a non-prerelease release publishing the release tag, the library version
tag and `latest` (4.4, 4.5, 6.3, 7.4), the concurrency group serializing runs (4.6), and an anonymous
pull once the packages are public (5.1, 5.2).

### After the merge

- Pull-request evidence (runs `35447446842` for the first commit and `35447902304` for the second): every
  image check passed - the library version cross-check reported `1.1.1`, the image reported `1.1.1`, the
  non-root check, the config check, the refresh-gated single pass and the compose smoke job - and the
  `publish` job was skipped. Of the pull request's full check list only `SonarCloud` and
  `SonarCloud Code Analysis` failed, and they also failed on the previous master commit `3f0c5c454`, so
  that failure is pre-existing and unrelated.
- Merge `fe874d226`: the push-to-master run `35449170897` passed the build and smoke checks and skipped
  publication.
- The snapshot release that `Release Latest` created after the merge (`created=2026-09-19T14:34:44Z`)
  triggered no image run at all, which satisfies "snapshot activity publishes nothing" (4.7) more
  strongly than required - but it also exposed the defect below.
- A `workflow_dispatch` run of the image workflow on master was started on demand to exercise the manual
  path (6.2); it publishes nothing because publishing was left disabled.

### The defect the merge exposed

The `release: published` trigger never fires for this repository's own releases: both release workflows
create the release with `JRELEASER_GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}`, and GitHub starts no
workflow run for events caused by the default token. Evidence: the released object exists, the trigger is
present on master, and `gh run list --workflow mapgen_image.yml --event release` is empty. As merged, a
release would therefore have published no image, which is the spec requirement
`container-image-publishing` "Tag set published for a release". Design decision 9 resolves it: `release.yml`
dispatches the image workflow on the released tag (`workflow_dispatch` is exempt from the token rule),
the workflow gained a `release` input to publish the release tag set, and the event trigger stays for
releases created by hand. Task group 7 implements and records it.

Tasks 6.2, 6.3, 6.4 and 7.4 stay open: they are runner- and release-side checks by definition (the manual
path in addition to the pull-request run, the first release after the merge, and the comparison of a
published image's `--tool-version` with the tag and with a generated `db.json`), so they can only be
completed once a release has been cut.
