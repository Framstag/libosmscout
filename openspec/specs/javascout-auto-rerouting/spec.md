...
Automatic route recalculation when the navigation engine detects the vehicle has left the planned route. Builds on the off-route detection provided by `javascout-navigation` — the engine emits `onRerouteRequest`, this capability defines what the app does in response.
