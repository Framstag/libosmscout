...
The `location-description-mapper` capability provides a three-layer JSON mapping from `LocationDescription` and its sub-types to MCP-compatible JSON structures (`content[]` for human-readable text, `structuredContent` for typed fields). It lives in `namespace osmscout::mcp` as free functions, independent of any HTTP framework.
