...
The `location-description-tool` capability provides an MCP tool handler that accepts latitude/longitude coordinates, calls `LocationDescriptionService::DescribeLocation()`, and returns structured description results via the `LocationDescriptionMapper`. It validates input and maps errors to appropriate HTTP status codes.
