# locationlookup-ranking-display Specification

## Purpose
The LocationLookup demo displays search results rank-sorted with an explicit rank column showing `typeRank × distanceRank × matchRank` and its components, matching the OSMScout2/JavaScout ranking. Rank weights and the search center are configurable.
