...
Route start/destination input UI for JavaScout.

## Purpose

Provide a route panel with two location input fields (start and destination) that reuse the existing search overlay pattern for location selection. Supports text search, map long-press, and read-only location summaries.

## Requirements

### Requirement: Route start/destination input

JavaScout SHALL provide a route panel with two location input fields (start and destination) and a vehicle selector. Each location field SHALL reuse the search overlay pattern — user taps a field to open the existing location search, selects a result, and the location is shown as a read-only summary label. The vehicle selector SHALL offer car, bicycle, and pedestrian options with optional avoid-* checkboxes. The search dialog SHALL pre-fill with the current field value when changing an existing location.
...
- **WHEN** user clicks the vehicle selector in the route panel
- **THEN** a dropdown or segmented control shows "Car", "Bicycle", and "Pedestrian" options
- **WHEN** user selects "Bicycle"
- **THEN** the route panel stores `RoutingProfile(Vehicle.BICYCLE)` as the active profile

#### Scenario: Toggle avoid tolls
- **WHEN** user checks the "Avoid tolls" checkbox
- **THEN** the route panel sets `avoidTolls = true` in the active profile
- **WHEN** user unchecks it
- **THEN** `avoidTolls` is set to false

#### Scenario: Toggle avoid ferries
- **WHEN** user checks the "Avoid ferries" checkbox
- **THEN** the route panel sets `avoidFerries = true` in the active profile
- **WHEN** user unchecks it
- **THEN** `avoidFerries` is set to false

#### Scenario: Profile passed to route calculation
- **WHEN** user clicks "Calculate" with a non-default profile selected
- **THEN** the route panel passes the `RoutingProfile` to `calculateRouteAsync`
- **AND** the route is calculated using the selected vehicle and avoid flags
