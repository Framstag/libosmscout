...
Provide CMake and Meson build integration for the Skia map rendering backend, including dependency detection, option flags, and conditional compilation.
...
The Meson build SHALL detect Skia via `dependency('skia', required: false)` with fallback to manual header/library detection.
