...
A manually invoked comparison application that renders given text with each available Linux backend, draws the text and its bounding boxes onto a stored canvas, dumps the measured values to the terminal, and compares them against an independent reference measurement.
