...
Cache rendered map tiles in an LRU map for potential future use (e.g., offline cache). Tiles are NOT used for display composition — they are keyed by pixel coordinates, not geographic position, so a composite would return stale content after a pan.
