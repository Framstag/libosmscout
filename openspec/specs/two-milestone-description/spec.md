...
The `two-milestone-description` capability extends `LocationDescriptionService` to describe a location as between two highway milestones on the same way, when two milestones bracket the location along the way topology. This builds on the single-milestone description capability by identifying both the previous (behind) and next (ahead) milestone along the way node order.
